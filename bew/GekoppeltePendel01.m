...
% Vergleich analytische und numerische Lösung für gleiche Pendel
% kleine Winkel
% -------------------------------------------------------------------------
clc
clear all
...
% Konstanten für Pendel gleicher Länge und Masse
L1 = 6;
L2 = 6;
m1 = 3;
m2 = 3;
...
omega2 = sqrt((omegaI^2+omegaII^2+kappa1+kappa2)/2+0.5*...
               sqrt((omegaI^2-omegaII^2)^2+(kappa1+kappa2)^2+ ...
                 2*(omegaI^2-omegaII^2)*(kappa1-kappa2)));
theta1 = rad2deg(C/2*(cos(omega1*time)+cos(omega2*time)));
theta2 = rad2deg(C/2*(cos(omega1*time)-cos(omega2*time)));
% theta1 = rad2deg(C*(cos((omega2-omega1)*time/2).*cos((omega2+omega1)*time/2)));
% theta2 = rad2deg(C*(sin((omega2-omega1)*time/2).*sin((omega2+omega1)*time/2)));
figure()
hold on
