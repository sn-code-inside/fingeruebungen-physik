...
% Reibungsparameter
eta1 = 0.25;  % für psi, phi
eta2 = 0.2;   % für theta
% Scheibe
[t1,Y1] = ode45(@DGL_Scheibe,tv,Y0,options,g,R,eta1,eta2);
% Reifen
[t2,Y2] = ode45(@DGL_Reifen,tv,Y0,options,g,R,eta1,eta2);
...
function dYdt = DGL_Scheibe(t,Y,g,R, eta1, eta2)
    % Y(1) = phi
    % Y(2) = theta
    % Y(3) = psi
    % Y(4) = d(phi)/dt
    ...
    dYdt = [Y(4)
            Y(5)
            Y(6)
            2*Y(5)*Y(6)/sin(Y(2))-eta1*Y(4)
            -sin(Y(2))*cos(Y(2))*Y(4)^2-6*sin(Y(2))*Y(4)*Y(6)/5.0-4*g/R ...
              *cos(Y(2))/5. - eta2*sign(Y(5))*Y(5)*Y(5)
...
function dYdt = DGL_Reifen(t,Y,g,R, eta1, eta2)
    % Y(1) = phi
    % Y(2) = theta
    % Y(3) = psi
    % Y(4) = d(phi)/dt
    ...
    dYdt = [Y(4)
            ...
            -sin(Y(2))*cos(Y(2))*Y(4)^2-4*sin(Y(2))*Y(4)*Y(6)/2.0-2*g/R ...
              *cos(Y(2))/3. - eta2*sign(Y(5))*Y(5)*Y(5)
