...
% Konstante Rotationsperiode T =300 S

phi0 = 90;                         % Abfahrtswinkel in °


% Fahrzeit ohne Reibung *1.5
tmax = 3*2*RK/v0;
...
titlestr2= strcat(" {\eta} \rm{=}" ,num2str(eta,' %3.2e 1/s'));
titlestr3= strcat(" {T_0} \rm{=}" ,num2str(T0,' %3.0f s'));
title(titlestr1+titlestr3,'FontWeight','normal','FontSize',14);
set(gca,'Fontsize', 16);
...
legend(phistr, 'location','southeast','NumColumns',2);
legend box off;
titlestr1= strcat("\phi \rm{=}",num2str(phi,' %3.0f°s    '));
titlestr2= strcat(" {\eta} \rm{=}" ,num2str(eta,' %3.2e 1/s'));
titlestr3= strcat(" {T_0} \rm{=}" ,num2str(T0,' %3.0f s'));
title(titlestr1+titlestr3,'FontWeight','normal','FontSize',14);
set(gca,'Fontsize', 16);
...
function dY = CoriolisDGLS(t, Y, Omeg, OmegDot, Eta)
    dY    = zeros(4,1);
    dY(1) = Y(2);
    dY(2) = -Omeg^2*Y(1) + 2*Omeg*Y(4) + OmegDot*Y(1)- Eta*Y(2);
    dY(3) = Y(4);
    dY(4) = -Omeg^2*Y(3) - 2*Omeg*Y(2) - OmegDot*Y(3)- Eta*Y(4);
end
