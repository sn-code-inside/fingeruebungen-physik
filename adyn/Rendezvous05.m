...
% und Jägers (Lösung über die Kepler-Gleichung bzw. Lagrange-Gleichung).
% -------------------------------------------------------------------------
...
ecc = 0.200;            % Elliptizität der Raumtransporter (Jäger) Bahn%% 
...
%% Numerische Lösung volle Lagrange-Gleichung für Relativkoordinaten

% AB
y0 = (y(1)-Y(1));
x0 = 0;
dx0 = sqrt(GA*ME/R0A)*(sqrt((1+ecc)/(1-ecc))-(1-ecc));
dy0 = 0;

yW0 = [x0 dx0 y0 dy0]; % Anfangsvektor
tmax  = T0;           % Simulationszeit in s
tspan = linspace(0,tmax,121);
t= tspan;

opt=odeset('AbsTol',1.e-9,'RelTol',1.e-8);
[t,yW]=ode45(@dgl_LGL,tspan,yW0,opt,w0,R0A); 

% Lösung_LGL über ODE45
xRnum  = yW(:,1);
yRnum  = yW(:,3);
...
dy0 = 0;
% CW-Gleichungen
xRCW  = (6*y0+4*dx0/w0)*sin(w0*t)+2*dy0*cos(w0*t)/w0-2*dy0/w0 ...
            -(3*dx0+6*w0*y0)*t + x0;
yRCW  = -(3*y0+2*dx0/w0)*cos(w0*t)+dy0*sin(w0*t)/w0+4*y0+2*dx0/w0;
...
axis ([-10 10 -10 10]*1.5)          %windows size
grid on
...
% end
hp(6) = plot(xRCW*1e3,yRCW*1e3,'+','MarkerIndices',1:10:length(x),...
    'color',Colors(4,:),'LineWidth',2,'LineStyle',Style(2));
zaehl = 0;
for k=1:10:length(X)-1
    zaehl = zaehl +1;
    hl(k)=LabelPoints(xRCW(k)*1e3, yRCW(k)*1e3, ...
        num2str(zaehl),'N',0.25,'FontSize',14,'Color',Colors(4,:));
end
...
axis ([-1 1 -1 1]*5.0e3)           %windows size
grid on
hp(7) = plot(0,0,'s','MarkerSize',10,'color',Colors(4,:),'LineWidth',3);
legend(hp(4:7),'Kepler-Gleichung','Lagrange-Gleichung',...
       'CW-Gleichungen','Target',...
...
function dY = dgl_LGL(t,yW,w0,R0A)
    Y = R0A*cos(w0*t);
    X = R0A*sin(w0*t);
    x = yW(1); dx = yW(2); y = yW(3); dy = yW(4);
    abst = sqrt((x)^2+(y+R0A)^2);  
    fac = (1-R0A^3/abst^3)*w0^2;
    dY = [  dx
            fac*(+x)-2*w0*dy
            dy
            fac*(R0A+y)+2*w0*dx
