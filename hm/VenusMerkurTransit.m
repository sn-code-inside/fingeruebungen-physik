...
% Die Zeiten müssen näherungsweise z.B. aus InnerePlanetenPQR.m bekannt
% sein. Daraus wird die Astronomische Einheit aus den Daten eines Venustransits
% bestimmt.
% a) Parallaxen-Methode: Die beobachteten Werten für zwei geographischen
...
%Koordinaten der beiden Orte Tromsö und Canberra
RE = 6378;
phi(1)= 35.28 ;
lambda(1)= 18.95;
phi(2) = -35.28;
...
[alpha,delta]=KeplerSonne(T1, eps);
ekl_Sun = CalcXYZfromAngles([1; alpha; delta]);
eklT=CalcXYZfromAngles([1; theta(1); deg2rad(phi(1))]);
eklC=CalcXYZfromAngles([1; theta(2); deg2rad(phi(2))]);
verb=eklT-eklC;
...
gamma  = (TE^2/TV^2)^(1/3);
OmegaB = 2*pi*(1/TV -1/TE)/86400/(gamma-1); %Basis Winkelgeschw. der Projektion
T_max  = bw/OmegaB;
ls     = 0.775;
T_1    = ls*T_max;
T_TR   = 21780;
...
for k=1:5 
   lgdtitle(k,:) = strjoin(["\it d\rm_{eff} = ", num2str(round(d_effv(k)),'%4d'),...
                         "     \it l\rm_S = ", num2str(lsv(k),'%4.3f')]);
end
...
figure('Name', "AE Bestimmung über Kontaktzeitmethode"),
subplot(1,2,1);
