...
figure('Name',header);
p(1)=plot3(YB1(1,1),YB1(1,2),YB1(1,3),'Color', Colors(2,:));
p(1).Marker='o';
p(1).LineWidth=1;
hold on
text(YB1(1,1),YB1(1,2),-0.03, BaPaK.Name(NrK),'HorizontalAlignment',...
    'center','FontSize',14,'Color', Colors(2,:));
p(5)=plot3(Planets(5).xyz(1,1),Planets(5).xyz(2,1), ...
              Planets(5).xyz(3,1),'Color', Colors(5,:));
p(5).LineWidth=1;
p(5).Marker='o';
text(-4 ,2, +0.03,'Jupiter',...
     'HorizontalAlignment','right','FontSize',14,'Color', Colors(5,:));
SonneFP;
xlabel('x in AE')
ylabel('y in AE');
zlabel('z in AE');
...
ylim([-6 2]);
zlim([-0.2 0.2]);
pbaspect([1 1 0.6])                    %Aspektverhältnis Achsen
xl = xlim;
yl = ylim;
[X,YB] = meshgrid(xl,yl);
surf(X,YB,zeros(size(X)))
...
for k1=1:length(YB1(:,1))-1
 ...
 zlim([-0.2 0.2]);
 pbaspect([1 1 0.6])                    %Aspektverhältnis Achsen
 p(1)=plot3(YB1(k,1),YB1(k,2),YB1(k,3),'Color',Colors(2,:));
 p(1).LineWidth=1;
 p(1).Marker='+';
 for j=3:5
     p(j)=plot3(Planets(j).xyz(1,k),Planets(j).xyz(2,k), ...
                  Planets(j).xyz(3,k),'Color', Colors(j,:));
     p(j).LineWidth=1;
     p(j).Marker='.';
 end
 pause(0.05);
